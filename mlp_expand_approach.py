...
class MLP(nn.Module):
    def __init__(self, inp_size, out_size, hidden_units, drop_rates):
        ...
        linears.append(nn.BatchNorm1d(num_features=out_size))
...
def run():
    ...
    # clf = MLP(inp_size=390, out_size=5, hidden_units=hidden_units, drop_rates=drop_rates).cuda()
    clf = torch.load('checkpoints/mlp-spand-1000.pkl').cuda()
    clf.train()
    ...
    for e in range(epoch):
        ...
        if (e+1) % 100 == 0:
            torch.save(clf, f'checkpoints/mlp-spand-{e+1}.pkl')
