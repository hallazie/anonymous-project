...
from mlp_encoder_approach import MLP
from blocks import AutoEncoder, DynamicGNoise
...
class Tester:
    ...
    def test(self):
        encoder = torch.load('checkpoints/autoencoder-500.pkl').cuda()
        for p in encoder.parameters():
            p.requires_grad = False
        encoder.eval()

        clf = torch.load('checkpoints/mlp-2400.pkl')
        for param in clf.parameters():
            param.requires_grad = False
        clf.eval()

        enco, _ = encoder(torch.from_numpy(self.loader.X_test).float().cuda())
        pred = clf(enco).cpu().detach().numpy()

        # pred = np.where(pred[:, 3] > 0.5, 1, 0).astype('int')
        # labl = self.loader.y_test[:, 3]

        # # mean first then thresh, 200e=660, 0.53, 565, 0.529, 571, 0.528, 600, 0.530
        # pred = np.where(np.mean(pred, axis=1) > 0.5, 1, 0).astype('int')

        # # use only resp for thresh, 200e=524, 0.530
        # pred = np.where(pred[:, 0] > 0.5, 1, 0).astype('int')

        # thresh first then mean then thresh=764, 777, 0.529, 589, 648, 0.53
