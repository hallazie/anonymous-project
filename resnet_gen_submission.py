...
prefix_list = [x.split('_')[0] for x in os.listdir(data_root) if x.endswith('red.png')][:20]
...
for i in range(len(seg_nuc)):
    ...
    for j in range(len(single_masks)):
        ...
        if not valid_size(cropped.shape):
            # print(f'invalid shape: {cropped.shape}')
            continue
        ...
        for e in pred_idx:
            pred_count[e] += 1
    pred_curr = [k for k, v in pred_count.items() if float(v)/len(single_masks) > 0.3]
    label_idx = label_dict.get(prefix, [])
    print(f'id={prefix}, pred={sorted(pred_curr)}, label={sorted(label_idx)}')
